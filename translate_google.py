...
def any_to_any_translate(google_tokn, content, from_='zh-TW', to_='en'):
    """
       自定义选择
    ...
    url = "http://translate.google.cn/translate_a/single?client=t&sl={0}&tl={1}" \
          "&hl=zh-TW&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&" \
...
def any_to_any_translate_back(google_tokn, content, from_='zh-TW', to_='en'):
    """
      中英，英中回译
    :param content:str, 4891个字， 用户输入
    :param from_: str, original language
...
if __name__ == '__main__':
    ...
    aug_sentence = set()
    while True:
        sen_org = input("請輸入要擴寫的句子(Back Translation) : \n")
        with open("擴寫句子.txt", "w") as in_file :
            in_file.write('原句 : ' + sen_org + '\n')
            for language_short_google_one in language_short_google:
                text_translate = any_to_any_translate_back(google_tokn, sen_org, from_='zh-TW', to_=language_short_google_one)
                print(language_short_google_one + " : ")
                print(text_translate.encode('utf8', 'ignore').decode('utf8'))
                if not text_translate in aug_sentence :
                    aug_sentence.add(text_translate)
                    in_file.write(text_translate + '\n')
                    # in_file.write('\n')
